...
const state = {
    ...
    /* NEW: currently logged-in vendor */
    currentVendor: {
        id: "vendor_001",
        name: "Luxury Couture"
    },
...
elements.vendorPanel.classList.toggle('active');

if(elements.vendorPanel.classList.contains("active")){
...
function renderNewArrivals(){
    ...
    container.innerHTML = newest.map((product, index) => `
        <div class="product-card" ${index < 4 ? 'data-new' : ''}>
            <div class="product-image">
                <img src="${product.image}" alt="${product.name}">
            ...
            <div class="product-info">
                ...
                onclick="viewProductFromArrivals(${product.id})">
                    VIEW PRODUCT
                </button>
            </div>
        </div>
...
function viewProductFromArrivals(id){
...
vendorId: state.currentVendor.id,
...
const vendorOrders = state.orders.filter(order =>
order.items.some(item => {
const product = state.products.find(p => p.id === item.productId);
return product.vendorId === state.currentVendor.id;
})
);
...
const vendorProducts = state.products.filter(product =>
product.vendorId === state.currentVendor.id
);
...
<button class="vendor-action-btn vendor-edit-btn"
onclick="editVendorProduct(${product.id})">
Edit
</button>

<button class="vendor-action-btn vendor-delete-btn"
...
function showVendorSection(section){

document.querySelectorAll('.vendor-menu li').forEach(li=>{
...
'vendor' + section.charAt(0).toUpperCase() + section.slice(1)
).classList.add('active');

}

function openVendorProductModal(){
document.getElementById("vendorProductModal").classList.add("active");
}

function closeVendorProductModal(){
document.getElementById("vendorProductModal").classList.remove("active");
}
...
return product && product.vendorId === state.currentVendor.id;
...
<select onchange="updateVendorOrderStatus('${order.id}',this.value)">
...
function updateVendorOrderStatus(orderId,status){
...
openVendorProductModal();
 ...
 function createParticles() {
        ...
        window.addEventListener('load', () => {
            setTimeout(() => {
                completeLoading();
            }, 2500); // Adjust timing as needed (matches progress bar)
        });
        ...
        function completeLoading() {
            ...
            // Add exiting class for smooth fade out
        ...
        // Optional: Click to skip loader (for testing)
        document.getElementById('loader-overlay').addEventListener('click', () => {
            completeLoading();
        });